...
final class SMTPToolTests: XCTestCase {
    func testSendTextMessageOverTSL() throws {
        ...
        defer { app.shutdown() }
        try configure(app)
        let tslSmtpConfiguration = SmtpConfiguration(hostname: Environment.get(NeededEnvironmentVariables.SMTPTOOL_SMTP_SERVER.rawValue)!,
                                                     port: Int(Environment.get(NeededEnvironmentVariables.SMTPTOOL_SMTP_PORT.rawValue)!)!,
                                                   signInMethod: .credentials(username: Environment.get(NeededEnvironmentVariables.NO_REPLY_EMAIL.rawValue)!, password: Environment.get(NeededEnvironmentVariables.NO_REPLY_PASSWORD.rawValue)!),
