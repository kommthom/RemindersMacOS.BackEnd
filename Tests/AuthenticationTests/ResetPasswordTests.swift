...
final class ResetPasswordTests: XCTestCase {
    ...
    func testResetPassword() throws {
        ...
        let resetPasswordRequest = ResetPasswordRequest(email: "tbenninghaus@gmx.de")
        try app.test(.POST, "api/auth/reset-password", content: resetPasswordRequest, afterResponse: { res in
            ...
            XCTAssertEqual(resetPasswordJob.recipient.address, "tbenninghaus@gmx.de")
            XCTAssertEqual(resetPasswordJob.templateName, EmailTemplateType.resetPasswordEmail.templateName)
    ...
    func testRecoverAccount() throws {
        let user = UserModel(fullName: "Test User", email: "test@test.com", passwordHash: "oldpassword")
        try app.repositories.users.create(user).wait()
