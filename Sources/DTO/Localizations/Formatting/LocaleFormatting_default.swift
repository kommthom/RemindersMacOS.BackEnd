//
//  LocaleFormatting_default.swift
//
//
...
public struct LocaleFormatting_default: LocaleFormattingProtocol {
    ...
    public func dateParseStrategy(formatType: CustomDateFormatType) -> Date.ParseStrategy? {
        return switch formatType {
            ...
            case .mmyyyy:
                Date.ParseStrategy(format: "\(year: .padded(4))-\(month: .twoDigits)-", locale: locale, timeZone: timeZone)
            case .ddmmyyyy:
            Date.ParseStrategy(format: "\(year: .padded(4))-\(month: .twoDigits)-\(day: .twoDigits)", locale: locale, timeZone: timeZone)
            default: nil
        }
    }
