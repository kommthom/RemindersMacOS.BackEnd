...
//  Originally created by David Bureš on 03.07.2022.
//
...
func shell(
    ...
    /*if let proxySettings = AppConstants.proxySettings {
        AppLogger.backend.log("Proxy is enabled")
    ...
    do {
        ...
        Logger(label: "reminders.backend").error("\(error.localizedDescription)")
    }
    return AsyncStream { continuation in
        pipe.fileHandleForReading.readabilityHandler = { handler in
            guard let standardOutput = String(data: handler.availableData, encoding: .utf8) else { return }
            guard !standardOutput.isEmpty else { return }
            continuation.yield(.standardOutput(standardOutput))
        }
        errorPipe.fileHandleForReading.readabilityHandler = { handler in
            guard let errorOutput = String(data: handler.availableData, encoding: .utf8) else { return}
